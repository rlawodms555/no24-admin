				...
				<form id="show-form" method="post" action="register.do" enctype="multipart/form-data">
			    	<div class="form-group mt-4">
				    	...
				    	<select name="topCategoryNo" id="box-big" style="width: 200px; height: 30px; margin-top: 14px; margin-bottom: 14px;">
		    				...
		    				<option value="1" ${show.no eq '1' ? 'selected' : '' }>콘서트</option>
		    				<option value="2" ${show.no eq '2' ? 'selected' : '' }>뮤지컬</option>
		    				<option value="3" ${show.no eq '3' ? 'selected' : '' }>연극</option>
		    				<option value="4" ${show.no eq '4' ? 'selected' : '' }>클래식</option>
		    				<option value="5" ${show.no eq '5' ? 'selected' : '' }>전시</option>
		    				<option value="6" ${show.no eq '6' ? 'selected' : '' }>아동</option>
						</select>
