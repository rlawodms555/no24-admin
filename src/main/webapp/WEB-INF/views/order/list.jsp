...
<body style="font-family: 'Nanum Gothic', sans-serif;">
	...
	<div class="container">
		...
		<h2 class="w3-border-bottom w3-border-light-grey w3-padding-16"><i class='far fa-file-alt' style="color: orange; margin-right: 10px;"></i>예매조회</h2>
  		<div class="row mt-5 mb-5">
  			<div class="col-12 d-flex justify-content-center">
				...
				<select name="opt" style="margin-right: 10px;">
					<option value="id" ${param.opt eq 'title' ? 'selected' : '' }> 아이디</option>
             		<option value="name" ${param.opt eq 'content' ? 'selected' : '' }> 이름</option>
				</select>
        ...
        <div class="mb-1">
	  	...
	  	<table class="table">
		    ...
		    <c:forEach var="order" items="${orders }">
		    	<tr>
		      		<td><input type="checkbox" name="noticeNo" value="${order.no }"></td>
		        	<td><a href="detail.do?orderNo=${order.no }&userNo=${order.userNo }" class="btn btn-outline-danger btn-sm">${order.no }</a></td>
		        	<td>${order.id }</td>
		        	<td>${order.totalOrderPrice }</td>
		        	<td>${order.paymentMethod }</td>
		        	<td><p class="text-primary">${order.totalPaymentPrice }</p></td>
		...
		<div class="row float-right mt-3">
			...
			<button type="button" class="btn text-danger" id="delete-checked">
				<strong><i class='fas fa-trash-alt' style='font-size:24px; margin-right: 10px;'></i>예매취소</strong>
			</button>
			</div>
		</div>
		<br>
